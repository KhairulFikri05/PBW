...
class CheckoutController extends Controller
{
    ...
    // 🌟 JALAN NINJA: LANGKAH 3 YANG KITA TAMBAHKAN 🌟
    public function paymentSuccess(Request $request)
    {
        // Tangkap order_id dari URL (?order_id=xxx) hasil kiriman JavaScript checkout
        $orderId = $request->query('order_id');

        if ($orderId) {
            // Cari data order-nya di database server
            $order = Order::find($orderId);
            
            if ($order) {
                // Paksa statusnya berubah jadi Lunas dan Masuk Dapur lewat browser user
                $order->update([
                    'payment_status' => 'paid',
                    'status' => 'dimasak' 
                ]);
        ...
        // Kembalikan ke halaman utama dengan pesan sukses alert laravel
        return redirect('/')->with('success', 'Pembayaran berhasil dikonfirmasi! Pesanan Anda sedang disiapkan di dapur.');
    }
